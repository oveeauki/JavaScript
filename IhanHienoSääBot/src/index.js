/**
@author oveeauki
@description HienoSääBotti for Discord
...
import {algos,bytestuff} from "../Modules/Randalgos.js"
import {hashopt,help} from "../Modules/help.js"

const client = new Discord.Client();
const api = new API_Obj();
const algs = new algos(), bts = new bytestuff();
 ...
 /*----------------------------------------------------------------------------------------*/
  ...
  if(message.content.startsWith(pfx1) && !message.author.bot){
    ...
    switch(spltfinal[0]){
      ...
      case "shift":
        const in_ = msgfinal.replace(/shift/gi,"").trim().split(" ");
        const res = await bts.bitshift(in_);
        await message.reply(res);
        break;

      case "and":
          try{
            const in_ = msgfinal.replace(/and/gi,"").trim().split(" ");
            const res = await bts.and_or_not(in_);
            await message.reply(res);
            break;
          }catch{Error}
      ...
      case "delete":
          ...
          else await message.reply("<:bro:968649274281840640>");
            }catch{Error}
            break;
